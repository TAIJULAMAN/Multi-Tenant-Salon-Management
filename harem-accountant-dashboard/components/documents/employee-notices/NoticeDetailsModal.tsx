...
export default function NoticeDetailsModal({ isOpen, onClose, notice }: NoticeDetailsModalProps) {
  useEffect(() => {
    if (isOpen) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-white border border-slate-100 rounded-2xl shadow-2xl z-10 flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-slate-100">
          <h3 className="text-base font-bold text-slate-800">Notice Details</h3>
          <button 
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          {/* Header Info */}
          <div>
            <h4 className="text-lg font-bold text-slate-800 mb-4">{notice.title}</h4>
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              ...
              <div className="text-right">
                <div className="text-xs font-bold text-slate-500 mb-1">{notice.dateSent}</div>
          ...
          <div className="text-sm text-slate-600 leading-relaxed font-medium">
            <p>
              Hi {notice.recipient.name.split(" ")[0]},<br /><br />
              Please review the attached document regarding the {notice.title.toLowerCase()}. It requires your attention by the end of the week.<br /><br />
          ...
          {/* Attachment */}
          <div>
            <div className="text-xs font-bold text-slate-800 mb-3">Attachments (1)</div>
            <div className="flex items-center justify-between p-3 rounded-xl border border-slate-100 bg-slate-50 hover:bg-slate-100/50 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div className="text-xs font-bold text-slate-700">{notice.title.toLowerCase().replace(/\s+/g, "_")}.pdf</div>
              </div>
