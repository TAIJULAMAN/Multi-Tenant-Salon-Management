...
const CustomCheckbox = ({ checked, onChange }: { checked: boolean; onChange: () => void }) => (
  <div 
    ...
    className={`w-5 h-5 rounded-[6px] flex items-center justify-center cursor-pointer transition-all duration-200 ${
      checked ? "bg-[#5c60f5] shadow-sm" : "bg-white border-2 border-slate-200 hover:border-[#5c60f5]"
    }`}
  >
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
      ...
      <div className="p-6 border-b border-slate-100 flex items-center justify-between">
        <h3 className="text-sm font-bold text-slate-800 tracking-tight">
          Imported List <span className="text-slate-400 font-medium ml-2">({data.length})</span>
        </h3>
                ...
                <tr key={payment.id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center justify-center gap-2">
                      <button onClick={() => handleReject(payment.id)} className="h-7 w-7 rounded bg-pink-50 text-[#ff4d79] flex items-center justify-center hover:bg-[#ff4d79] hover:text-white transition-colors cursor-pointer">
                        <X size={14} />
                      </button>
                      <button onClick={() => handleApprove(payment.id)} className="h-7 w-7 rounded bg-emerald-50 text-emerald-500 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition-colors cursor-pointer">
                        <Check size={14} />
                      </button>
                    </div>
                  </td>
